...
export function TidepoolSplash({ color = "#5dd6ff" }: { color?: string }) {
...
function paintScene(ctx: CanvasRenderingContext2D, width: number, height: number, time: number, palette: ReturnType<typeof createSplashPalette>, bands: CubeMoireBand[]) {
  ...
  for (let i = 0; i < pools; i += 1) {
    ...
    const y = height * (0.22 + Math.sin(t * Math.PI * 2 + time * 0.16) * 0.16);
    const field = sampleCubeField(bands, x, y, time);
    const radius = Math.min(width, height) * (0.03 + field * 0.04 + (i % 3) * 0.008);
    const orb = ctx.createRadialGradient(x, y, 0, x, y, radius * 2.8);
    ...
    ctx.strokeStyle = rgba(palette.highlight, 0.05 + field * 0.08);
    ctx.lineWidth = 1;
    ctx.beginPath();
    ctx.arc(x, y, radius * (1.1 + field * 0.12), 0, Math.PI * 2);
    ctx.stroke();
  }

  const sheen = ctx.createLinearGradient(0, 0, 0, height);
  sheen.addColorStop(0, rgba(palette.soft, 0.02));
  sheen.addColorStop(1, rgba(palette.deep, 0.2));
  ctx.fillStyle = sheen;
  ctx.fillRect(0, 0, width, height);
