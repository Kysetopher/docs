...
export function EstuarySplash({ color = "#5dd6ff" }: { color?: string }) {
...
function paintScene(ctx: CanvasRenderingContext2D, width: number, height: number, time: number, palette: ReturnType<typeof createSplashPalette>, bands: CubeMoireBand[]) {
  ...
  for (let i = 0; i < channels; i += 1) {
    const t = i / (channels - 1);
    const y = height * (0.16 + t * 0.68);
    const field = sampleCubeField(bands, width * 0.52, y, time);
    const bend = (field - 0.5) * height * 0.22;
    ctx.strokeStyle = rgba(i % 2 === 0 ? palette.primary : palette.secondary, 0.1 + field * 0.12);
    ctx.lineWidth = 1.2 + field * 1.5;
    ctx.beginPath();
    ctx.moveTo(0, y);
    ctx.bezierCurveTo(width * 0.28, y + bend, width * 0.66, y - bend * 0.6, width, y + bend * 0.2);
    ctx.stroke();
  }

  const mouth = ctx.createRadialGradient(width * 0.54, height * 0.58, 0, width * 0.54, height * 0.58, Math.max(width, height) * 0.8);
  mouth.addColorStop(0, rgba(palette.highlight, 0.08));
  mouth.addColorStop(0.55, rgba(palette.soft, 0.03));
  mouth.addColorStop(1, rgba(palette.deep, 0.22));
  ctx.fillStyle = mouth;
  ctx.fillRect(0, 0, width, height);
