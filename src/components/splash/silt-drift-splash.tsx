...
export function SiltDriftSplash({ color = "#5dd6ff" }: { color?: string }) {
...
function paintScene(ctx: CanvasRenderingContext2D, width: number, height: number, time: number, palette: ReturnType<typeof createSplashPalette>, bands: CubeMoireBand[]) {
  ...
  const dust = Math.max(160, Math.round((width * height) / 2200));
  for (let i = 0; i < dust; i += 1) {
    const nx = ((Math.sin(i * 12.1 + time * 0.18) + 1) * 0.5);
    const ny = ((Math.cos(i * 9.7 - time * 0.12) + 1) * 0.5);
    const x = nx * width;
    const y = ny * height;
    const field = sampleCubeField(bands, x, y, time);
    const size = 0.6 + field * 1.8;
    ctx.fillStyle = rgba(i % 2 === 0 ? palette.soft : palette.highlight, 0.02 + field * 0.05);
    ctx.beginPath();
    ctx.arc(x, y + Math.sin(time * 0.5 + i) * 3, size, 0, Math.PI * 2);
    ctx.fill();
  }

  const haze = ctx.createLinearGradient(0, 0, 0, height);
  haze.addColorStop(0, rgba(palette.highlight, 0.02));
  haze.addColorStop(1, rgba(palette.deep, 0.2));
  ctx.fillStyle = haze;
  ctx.fillRect(0, 0, width, height);
}
