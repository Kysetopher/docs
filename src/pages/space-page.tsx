    ...
    <DocsShell>
      <div className="h-full overflow-y-auto">
        <div className=" space-y-8">
          ...
          <section className="grid p-2 gap-2 sm:grid-cols-2 xl:grid-cols-3">
            {space.docs.map((doc) => (
              <Link
                key={doc.id}
                to={doc.href}
                className="group relative isolate block h-full border p-5 "
              >
                <div className="absolute left-5 top-0 z-[1] h-1 w-16 -translate-y-1/2 bg-background" />
                <div className="absolute z-[1] inset-0  bg-gradient-to-b from-background to-background/18"/>
                <div className="absolute inset-0  opacity-95">{getSpaceBanner(space.id)}</div>
                {doc.cardIcon ? (
